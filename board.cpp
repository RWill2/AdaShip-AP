...
bool Board::placeShip(int num, int x, int y, bool isHorizontal){
  if(x > boardWidth || y > boardHeight){
    ...
    shipPos[num].setPosition(x, y, isHorizontal);
    return true;
  
}

bool Board::recordHit(int x, int y){
  for(int e = 0; e < shipNames.size(); e++){
    if(shipPos[e].setHit(x,y)){
      gameBoard[x][y] = HIT;
      if(shipPos[e].getSunk()){
        std::cout << "You have sunk the " << shipNames[e] << std::endl;
        return true;
      }
    } else{
    gameBoard[x][y] = MISS;
